...
from arouse._dj.db.migrations.operations.base import Operation
from arouse._dj.db.migrations.utils import COMPILED_REGEX_TYPE, RegexObject
from arouse._dj.utils import datetime_safe, six
from arouse._dj.utils.encoding import force_text
from arouse._dj.utils.functional import LazyObject, Promise
from arouse._dj.utils.timezone import utc
from arouse._dj.utils.version import get_docs_version
...
class DatetimeSerializer(BaseSerializer):
    def serialize(self):
        ...
        if self.value.tzinfo is not None:
            imports.append("from arouse._dj.utils.timezone import utc")
...
class DeconstructableSerializer(BaseSerializer):
    ...
    def _serialize_path(path):
        ...
        if module == "django.db.models":
            imports = {"from arouse._dj.db import models"}
            name = "models.%s" % name
        else:
...
class OperationSerializer(BaseSerializer):
    def serialize(self):
        from arouse._dj.db.migrations.writer import OperationWriter
...
class SettingsReferenceSerializer(BaseSerializer):
    def serialize(self):
        return "settings.%s" % self.value.setting_name, {"from arouse._dj.conf import settings"}
...
def serializer_factory(value):
    from arouse._dj.db.migrations.writer import SettingsReference
