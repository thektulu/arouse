...
from arouse._dj.conf.locale import LANG_INFO
from arouse._dj.core.exceptions import AppRegistryNotReady
from arouse._dj.core.signals import setting_changed
from arouse._dj.dispatch import receiver
from arouse._dj.utils import lru_cache, six
from arouse._dj.utils._os import upath
from arouse._dj.utils.encoding import force_text
from arouse._dj.utils.safestring import SafeData, mark_safe
from arouse._dj.utils.six import StringIO
from arouse._dj.utils.translation import (
...
class DjangoTranslation(gettext_module.GNUTranslations):
    ...
    def __init__(self, language, domain=None, localedirs=None):
        ...
        if self.domain == 'arouse':
            if localedirs is not None:
                # A module-level cache is used for caching 'arouse' translations
                warnings.warn("localedirs is ignored when domain is 'arouse'.", RuntimeWarning)
                localedirs = None
        ...
        if self.__language == settings.LANGUAGE_CODE and self.domain == 'arouse' and self._catalog is None:
            self._catalog = {}
...
def check_for_language(lang_code):
    ...
    for path in all_locale_paths():
        if gettext_module.find('arouse', path, [to_locale(lang_code)]) is not None:
            return True
    return False
...
def templatize(src, origin=None):
    ...
    """
    from arouse._dj.template.base import (
